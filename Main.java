...
public class Main {
    ...
    public static void guestPanel(Scanner scanner, Guest guest) throws InvalidInputException { // guest panel which is only executed after guestAuth()
        ...
        while (true) {
            ...
            // Invoice viewing path
                ...
                for (Invoice inv : guest.getInvoices()) {
                    inv.printInvoiceDetails();
                }
            }
            ...
            // Invoice paying path
                ...
                // build list of ONLY unpaid invoices
                ArrayList<Invoice> unpaidInvoices = new ArrayList<>();
                ...
                while (true) {
                    ...
                    try {
                        index = readIntBetween(scanner, 1, unpaidInvoices.size());
                        break;
                    } catch (InvalidInputException e) {
                        System.out.println(e.getMessage());
                    }
                ...
                // select from unpaid list only
                Invoice selected = unpaidInvoices.get(index - 1);

                double remaining = selected.getTotalAmount() - selected.getPaidAmount();
                ...
                if (methodChoice == 1) {
                    ...
                    System.out.print("Enter cash confirmation (y/n): ");
                ...
                boolean success = selected.pay(amount, method, email, password);
    ...
    public static void adminPanel(Scanner scanner, Admin admin) {  // Admin panel which is only executed after adminAuth()
        ...
        while (true) {
            ...
            System.out.println("7: Update guest balance");
            System.out.println("0: Logout");

            int choice;

            try {
                choice = readIntBetween(scanner, 0, 7);
            } catch (InvalidInputException e) {
                System.out.println(e.getMessage());
                continue;
            }
            ...
            // update guest balance path
                ...
                admin.updateGuestBalance(username, newBalance);
